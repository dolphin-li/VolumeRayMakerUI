...
namespace mpu
{
	const static float INVALID_CELL_VALUE = 1e15f;
	const static float HOLE_CELL_VALUE = std::numeric_limits<float>::quiet_NaN();
	inline bool is_invalid_cell_value(float v)
	{
		return fabs(v - INVALID_CELL_VALUE) < 1e-5f;
	}
	inline bool is_hole_cell_value(float v)
	{
		return std::isnan(v);
	}
	...
	public:
		...
		* */
		...
		/**
		* generate 0-1 volume: 1 means occupied and 0 means empty
		* */
		void binaryFromObjMesh(const ObjMesh& mesh, ldp::Int3 res, 
		...
		void makeHole(const VolumeMask& mask);
