...
namespace ldp
{
	...
	inline std::string repath(std::string fullname, std::string oldpath, std::string newpath)
	{
		if (!strstr(fullname.c_str(), oldpath.c_str()))
			throw std::exception(("path not contained in full: " + fullname + " ## " + oldpath).c_str());
		return fullfile(newpath, fullname.substr(oldpath.size()));
	}
	...
	inline std::string parentpath(std::string path)
	{
		...
		size_t pos = pos1;
		if (pos >= path.size() || (pos2 > pos1 && pos2 < path.size()))
			pos = pos2;
		if (pos >= path.size())
			return "";
